<x-dashboard>
    <x-slot:dashboardContent>
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="bg-blue-100 shadow-md rounded-lg p-4">
                <div class="flex justify-center w-full">
                    <img src="{{asset('/uploads'.'/'.$dog->image_path)}}" class="w-auto h-64">
                </div>
            
                <h2 class="text-gray-700 text-lg font-bold mt-2">NPR.{{$dog->price}}</h2>
                <hr class="border-blue-800 my-2">
                <p class="text-blue-600 font-bold">{{$dog->name}}.</p>
                <p class="text-gray-600 text-sm">{{$dog->description}}.</p>
                <div class="w-full flex pt-2 justify-between items-center">
                    <span class="py-1 px-4 text-{{$dog->available==='yes'?'green':'red'}}-800 rounded-full font-bold text-sm bg-{{$dog->available==='yes'?'green':'red'}}-300"> {{$dog->available==='yes'?"Available":"Not Available"}} </span>
                    <p class="text-gray-500 text-sm font-semibold">Breed: {{strtoupper($dog->breed)}}</p>
                </div>
              </div>
            @endif
            @endforeach
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="bg-blue-100 shadow-md rounded-lg p-4">
                ...
                <div class="flex justify-between items-baseline">
                    <h2 class="text-gray-700 text-lg font-bold mt-2">NPR.{{$dog->price}}.</h2>
                    <p class="text-gray-600 text-sm">{{strtoupper($dog->breed)}}</p>
                </div>
                ...
                <div class="w-full flex pt-2 justify-between items-center">
                    ...
                    <button onclick="openModal({{$dog->id}})" class="text-sm px-2 py-1 hover:border-blue-500 border-2 border-blue-300 rounded-lg text-blue-700">Buy</button>
                    @endif
                </div>
              </div>
            @endif
        ...
        <div id="popupModal" class="hidden absolute top-0 bg-opacity-50 right-0 w-full h-full bg-blue-300 flex items-center justify-center">
            ...
            <form action="{{route('buydog')}}" method="POST">
                @csrf
